...
options via a `list` passed via the `n_perc_args` argument.  **Important
Notes:** The default settings for `tab_summary` is below.
```{r}
args(tab_summary)
```
...
Note that the `tab_summary` are made while using `with`.  Further explaination
for this follows.
