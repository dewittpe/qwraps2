...
will help to illustrate the ordering or results when using a factor or a
...
Engine configuration: the `vs` column is an integer vector for
indicating V-shaped or straight. The constructed column engine is a
factor the same information as a labeled factor.
...
Spambase (Hopkins and Suermondt 1999) is a useful data set for example
needed a binary outcome and several possible predictors. The data set
