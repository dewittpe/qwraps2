...
vignette data set for examples throughout this vignette. `mtcars2` is a
modified and extended version of the base R data set `mtcars` . For
details on the construction of the `mtcars2` data set please view the
vignette: `vignette("mtcars", package = "qwraps2")`
...
are other options for formatting character string:
...
the `frmtci` function. You an modify the formatting of printed result by
adjusting the arguments pasted to `frmtci`.
...
Let $\left\{ x_{1},x_{2},x_{3},\ldots,x_{n} \right\}$ be a sample of
size $n$ with $x_{i} > 0$ for all $i.$ Then the geometric mean,
$\mu_{g},$ and geometric standard deviation are

$$\mu_{g} = \left( \prod\limits_{i = 1}^{n}x_{i} \right)^{\frac{1}{n}} = b^{\sum\limits_{i = 1}^{n}\log_{b}x_{i}},$$
and
$$\sigma_{g} = b^{\sqrt{\frac{\sum\limits_{i = 1}^{n}{(\log_{b}\frac{x_{i}}{\mu_{g}})}^{2}}{n}}}$$
or, for clarity,
$$\log_{b}\sigma_{g} = \sqrt{\frac{\sum\limits_{i = 1}^{n}\left( \log_{b}\frac{x_{i}}{\mu_{g}} \right)^{2}}{n}}$$

When looking for the geometric standard deviation in R, the simple
`exp(sd(log(x)))` is not exactly correct. The geometric standard
deviation uses $n,$ the full sample size, in the denominator, where as
the `sd` and `var` functions in R use the denominator $n - 1.$ To get
the geometric standard deviation one should adjust the result by
multiplying the variance by $(n - 1)/n$ or the standard deviation by
$\sqrt{(n - 1)/n}.$ See the example below.
...
variance, and standard deviation for a numeric vector.
